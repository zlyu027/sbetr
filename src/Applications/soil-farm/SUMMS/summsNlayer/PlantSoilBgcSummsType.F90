...
#include "bshr_assert.h"
  ...
  character(len=*), private, parameter :: filename = &
       __FILE__
  public :: plant_soilbgc_summs_type

  type, extends(plant_soilbgc_type) :: &
    plant_soilbgc_summs_type
  ...
  end type plant_soilbgc_summs_type

  interface plant_soilbgc_summs_type
    module procedure constructor
  end interface plant_soilbgc_summs_type

  contains

  !-------------------------------------------------------------------------------
  type(plant_soilbgc_summs_type) function constructor()
  !
  ! !DESCRIPTION:
  ! create an object of type plant_soilbgc_summs_type.
  ! Right now it is purposely empty
   type(plant_soilbgc_summs_type), allocatable :: plants
   allocate(plants)
   constructor = plants
...
!         tracer_flx_vtrans_patch(p, id_trc_no3x) * natomw
  ...
  !----------------------------------------------------------------------
end module PlantSoilBgcSummsType
