module BgcReactionsSummsType

#include "bshr_assert.h"

  !
  ! !DESCRIPTION:
  ! Does microbial modeling but keeps the N/P dynamics below.
  !
  ...
  use BgcSummsType          , only : summsbgceca_type
  ...
  type, public, extends(bgc_reaction_type) :: &
    bgc_reaction_summs_type
     private
    type(summsbgceca_type), pointer :: summseca(:,:)
    type(JarBGC_forc_type), pointer :: summsforc(:,:)
  ...
  contains
    ...
    procedure, private :: set_summs_forc
    procedure, private :: retrieve_output
    procedure, private :: rm_ext_output
    procedure, private :: precision_filter
  end type bgc_reaction_summs_type

  interface bgc_reaction_summs_type
     module procedure constructor
  end interface bgc_reaction_summs_type

contains


  !-------------------------------------------------------------------------------
  type(bgc_reaction_summs_type) function constructor()
    !
    ! ! DESCRIPTION:
    !
    ! create an object of type bgc_reaction_summs_type.
    ! Right now it is purposely empty
   type(bgc_reaction_summs_type), allocatable :: bgc
  ...
  do j = lbj, ubj
    do c = bounds%begc, bounds%endc
      call this%summseca(c,j)%UpdateParas(summs_para, bstatus)
...
!-------------------------------------------------------------------------------
  ...
  if(.not. exit_spinup .and. betr_spinup_state/=0 .and. spinup_stage/=2) then
    do c = bounds%begc, bounds%endc
      dom_scalar(c)=1._r8/summs_para%spinup_factor(9)
    enddo
  endif

  if(betr_spinup_state/=0)then
    move_scalar(tracers%id_trc_Bm) = summs_para%spinup_factor(7)
    move_scalar(tracers%id_trc_som) = summs_para%spinup_factor(8)
    move_scalar(tracers%id_trc_dom)=summs_para%spinup_factor(9)
  endif
       ...
       do c = bounds%begc, bounds%endc
         ...
         if(spinup_stage/=2)then
           call rescale_tracer_group(c, j, tracers%id_trc_beg_Bm, &
             tracers%id_trc_end_Bm, nelm, 1._r8/summs_para%spinup_factor(7))

! the treatment of dom will be revised
!           call rescale_tracer_group(c, j, tracers%id_trc_beg_dom, &
!             tracers%id_trc_end_dom, nelm, 1._r8/summs_para%spinup_factor(9))
    ...
    !scale the state variable back to the slow space
     ...
     do j = lbj, ubj
       do c = bounds%begc, bounds%endc
         !som1
         call rescale_tracer_group(c, j, tracers%id_trc_beg_Bm, &
             tracers%id_trc_end_Bm, nelm, summs_para%spinup_factor(7))

         call rescale_tracer_group(c, j, tracers%id_trc_beg_som, &
             tracers%id_trc_end_som, nelm, summs_para%spinup_factor(8))

!         call rescale_tracer_group(c, j, tracers%id_trc_beg_dom, &
!             tracers%id_trc_end_dom, nelm, summs_para%spinup_factor(9))
...
! end of copying from zlyu/test1
  ...
  do j = lbj, ubj
    do p = 1, nactpft
      this%summseca(c_l,j)%competECA%mumax_minn_nh4_plant(p) = plantNutkinetics%plant_nh4_vmax_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%mumax_minn_no3_plant(p) = plantNutkinetics%plant_no3_vmax_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%mumax_minp_plant(p) = plantNutkinetics%plant_p_vmax_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%kaff_minn_no3_plant(p)= plantNutkinetics%plant_no3_km_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%kaff_minn_nh4_plant(p)= plantNutkinetics%plant_nh4_km_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%kaff_minp_plant(p)   = plantNutkinetics%plant_p_km_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%plant_froot_nn(p) = plantNutkinetics%plant_eff_ncompet_b_vr_patch(p,j)
      this%summseca(c_l,j)%competECA%plant_froot_np(p) = plantNutkinetics%plant_eff_pcompet_b_vr_patch(p,j)
    enddo

    !mineral surfaces
    this%summseca(c_l,j)%competECA%kaff_minn_nh4_msurf= plantNutkinetics%km_minsurf_nh4_vr_col(c_l,j)  !this is ignored at this moment
  ...
  subroutine Init_betrbgc(this, bounds, lbj, ubj, betrtracer_vars, namelist_buffer, bstatus)
    ...
    do j = lbj, ubj
      ...
      enddo
    enddo
    this%use_c13 = summs_para%use_c13
    ...
    itemp_mem=0
    dom_cnt = 0
    trcid =  betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+c_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, &
         trc_name='MONOC', is_trc_mobile=.true., is_trc_advective = .true., &
         ...
         trc_family_name='MONO')
    if(bstatus%check_status())return

    trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+n_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, &
         trc_name='MONON', is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
         trc_family_name='MONO')
    ...
    trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+p_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, &
         trc_name='MONOP', is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
         trc_family_name='MONO')
    ...
    if(this%use_c13)then
      trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+c13_loc-1
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, &
         trc_name='MONOC_C13', is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
         trc_family_name='MONO')
    ...
    if(this%use_c14)then
      trcid=betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+c14_loc-1
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, &
         trc_name='MONOC_C14', is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
         trc_family_name='MONO')
    ...
    !extracellular enzyme biomass
    dom_cnt=dom_cnt+1
    trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+c_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='ENZC', &
         is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
         trc_family_name='ENZ')
    ...
    trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+n_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='ENZN', &
         is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
    ...
    trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+p_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='ENZP', &
         is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
    ...
    if(this%use_c13)then
      trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+c13_loc-1
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='ENZC_C13',&
         is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom, trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
    ...
    if(this%use_c14)then
      trcid = betrtracer_vars%id_trc_beg_dom+dom_cnt*nelm+c14_loc-1
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='ENZC_C14', &
         is_trc_mobile=.true., is_trc_advective = .true., &
         trc_group_id = betrtracer_vars%id_trc_dom,  trc_group_mem = addone(itemp_mem), &
         is_trc_volatile=.false., is_trc_adsorb = .true., trc_adsorbid=addone(itemp_ads), &
         trc_adsorbgroupid=addone(itemp_ads_grp), trc_sorpisotherm='LANGMUIR',&
    ...
    !------------------------------------------------------------------------------------
    !define microbial group
    Bm_cnt=0;itemp_mem=0
    !microbial structural biomass
    trcid = betrtracer_vars%id_trc_beg_Bm+Bm_cnt*nelm+c_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='MICC', &
         is_trc_mobile=.true., is_trc_advective = .false., &
    ...
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='MICN', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm, trc_group_mem = addone(itemp_mem), &
         trc_family_name='MIC')
    if(bstatus%check_status())return
    ...
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='MICP', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm, trc_group_mem = addone(itemp_mem), &
         trc_family_name='MIC')
    if(bstatus%check_status())return
    ...
    if(this%use_c13)then
      ...
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='MICC_C13',&
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm, trc_group_mem = addone(itemp_mem), &
         trc_family_name='MIC')
      if(bstatus%check_status())return
    ...
    if(this%use_c14)then
      ...
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='MICC_C14', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm,  trc_group_mem = addone(itemp_mem), &
         trc_family_name='MIC')
      if(bstatus%check_status())return
    endif
    !microbial reserve biomass
    Bm_cnt=Bm_cnt+1
    trcid = betrtracer_vars%id_trc_beg_Bm+Bm_cnt*nelm+c_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='RESC', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm, trc_group_mem = addone(itemp_mem), &
         trc_family_name='RES')
    if(bstatus%check_status())return
    ...
    trcid = betrtracer_vars%id_trc_beg_Bm+Bm_cnt*nelm+n_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='RESN', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm, trc_group_mem = addone(itemp_mem), &
         trc_family_name='RES')
    if(bstatus%check_status())return
    ...
    trcid = betrtracer_vars%id_trc_beg_Bm+Bm_cnt*nelm+p_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='RESP', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm, trc_group_mem = addone(itemp_mem), &
         trc_family_name='RES')
    if(bstatus%check_status())return
    ...
    if(this%use_c14)then
      trcid = betrtracer_vars%id_trc_beg_Bm+Bm_cnt*nelm+c14_loc-1
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='RESC_C14', &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_Bm,  trc_group_mem = addone(itemp_mem), &
         trc_family_name='RES')
      if(bstatus%check_status())return
    ...
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='POLYC' ,     &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_som, trc_group_mem= addone(itemp_mem), &
         trc_family_name='POLY')
    if(bstatus%check_status())return

    trcid = betrtracer_vars%id_trc_beg_som+som_cnt*nelm+n_loc-1
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='POLYN'  ,    &
         is_trc_mobile=.true., is_trc_advective = .false.,  &
         trc_group_id = betrtracer_vars%id_trc_som, trc_group_mem= addone(itemp_mem), &
         trc_family_name='POLY')
    if(bstatus%check_status())return
    ...
    call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='POLYP' ,    &
         is_trc_mobile=.true., is_trc_advective = .false.,  &
         trc_group_id = betrtracer_vars%id_trc_som, trc_group_mem= addone(itemp_mem), &
         trc_family_name='POLY')
    if(bstatus%check_status())return
    if(this%use_c13)then
      ...
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='POLYC_C13' ,    &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_som, trc_group_mem= addone(itemp_mem), &
         trc_family_name='POLY')
      if(bstatus%check_status())return
    ...
    if(this%use_c14)then
      ...
      call betrtracer_vars%set_tracer(bstatus=bstatus,trc_id = trcid, trc_name='POLYC_C14' ,    &
         is_trc_mobile=.true., is_trc_advective = .false., &
         trc_group_id = betrtracer_vars%id_trc_som, trc_group_mem= addone(itemp_mem), &
         trc_family_name='POLY')
      if(bstatus%check_status())return
  ...
  !--------------------------------------------------------------------
  subroutine rm_ext_output(this, c, j, dtime, nstates, ystatesf, summsbgc_index, summs_forc, biogeo_flux)
  !
  ! DESCRIPTION
  ...
  type(JarBGC_forc_type)      , intent(in) :: summs_forc
  type(betr_biogeo_flux_type) , intent(inout) :: biogeo_flux
  ...
  associate(                         &
    ...
    frac_loss_lit_to_fire => summs_forc%frac_loss_lit_to_fire, &
    frac_loss_cwd_to_fire => summs_forc%frac_loss_cwd_to_fire, &
  ...
  subroutine InitCold(this, bounds, col, betrtracer_vars, biophysforc, tracerstate_vars)
    ...
    use betr_constants           , only : stdout              ! addd   -zlyu
    implicit none
    ! !ARGUMENTS:
    class(bgc_reaction_summs_type)   , intent(inout)    :: this
  ...
  !------------------------------------------------------------------------------
  subroutine set_summs_forc(this, bounds, col, lbj, ubj, jtops, num_soilc, filter_soilc, &
      biophysforc, plant_soilbgc, betrtracer_vars, tracercoeff_vars, tracerstate_vars, betr_status)
  !DESCRIPTION
  ...
  associate( &
     ...
     wood_beg =>  this%summsbgc_index%wood_beg  , &
     wood_end =>  this%summsbgc_index%wood_end  , &
     som_beg =>  this%summsbgc_index%som_beg    , &
     som_end =>  this%summsbgc_index%som_end    , &
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      c = filter_soilc(fc)
      if(j<jtops(c))cycle
      this%summsforc(c,j)%plant_ntypes = this%nactpft
      this%summsforc(c,j)%ystates(:) = 0._r8

      !litter
      this%summsforc(c,j)%ystates(litr_beg:litr_end)= & 
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_litr:betrtracer_vars%id_trc_end_litr)

      !wood
      this%summsforc(c,j)%ystates(wood_beg:wood_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_wood:betrtracer_vars%id_trc_end_wood)

      !som
      this%summsforc(c,j)%ystates(som_beg:som_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_som:betrtracer_vars%id_trc_end_som)
      !if(this%summsforc(c,j)%ystates(som_beg)<=tiny_cval)this%summsforc(c,j)%ystates(som_beg:som_end)=0._r8
      !dom
      this%summsforc(c,j)%ystates(dom_beg:dom_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_dom:betrtracer_vars%id_trc_end_dom)
      !if(this%summsforc(c,j)%ystates(dom_beg)<=tiny_cval)this%summsforc(c,j)%ystates(dom_beg:dom_end)=0._r8
      !microbial biomass
      this%summsforc(c,j)%ystates(Bm_beg:Bm_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_Bm:betrtracer_vars%id_trc_end_Bm)
      !if(this%summsforc(c,j)%ystates(Bm_beg)<=tiny_cval)this%summsforc(c,j)%ystates(Bm_beg:Bm_end)=0._r8
      !non-soluble phase of mineral p
      k1= betrtracer_vars%id_trc_beg_minp; k2 = this%summsbgc_index%lid_minp_secondary
      this%summsforc(c,j)%ystates(k2) = fpmax(tracerstate_vars%tracer_conc_mobile_col(c,j,k1))

      k1 = betrtracer_vars%id_trc_end_minp;   k2 = this%summsbgc_index%lid_minp_occlude
      this%summsforc(c,j)%ystates(k2) = fpmax(tracerstate_vars%tracer_conc_mobile_col(c,j,k1))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_n2) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_n2))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_o2) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_o2))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_ar) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_ar))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_co2)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_co2x))

      if(this%use_c13)then
        this%summsforc(c,j)%ystates(this%summsbgc_index%lid_c13_co2)= &
      ...
      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_nh4)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_nh3x))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_no3)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_no3x))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_n2o)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_n2o))

      this%summsforc(c,j)%ystates(this%summsbgc_index%lid_minp_soluble) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c,j,betrtracer_vars%id_trc_p_sol))

      !input
      this%summsforc(c,j)%cflx_input_litr_met = biophysforc%c12flx%cflx_input_litr_met_vr_col(c,j)
      this%summsforc(c,j)%cflx_input_litr_cel = biophysforc%c12flx%cflx_input_litr_cel_vr_col(c,j)
      this%summsforc(c,j)%cflx_input_litr_lig = biophysforc%c12flx%cflx_input_litr_lig_vr_col(c,j)
      ...
      this%summsforc(c,j)%nflx_input_litr_lig = biophysforc%n14flx%nflx_input_litr_lig_vr_col(c,j)
      this%summsforc(c,j)%nflx_input_litr_cwd = biophysforc%n14flx%nflx_input_litr_cwd_vr_col(c,j)
      this%summsforc(c,j)%nflx_input_litr_lwd = biophysforc%n14flx%nflx_input_litr_lwd_vr_col(c,j)
      this%summsforc(c,j)%nflx_input_litr_fwd = biophysforc%n14flx%nflx_input_litr_fwd_vr_col(c,j)

      this%summsforc(c,j)%pflx_input_litr_met = biophysforc%p31flx%pflx_input_litr_met_vr_col(c,j)
      this%summsforc(c,j)%pflx_input_litr_cel = biophysforc%p31flx%pflx_input_litr_cel_vr_col(c,j)
      this%summsforc(c,j)%pflx_input_litr_lig = biophysforc%p31flx%pflx_input_litr_lig_vr_col(c,j)
      this%summsforc(c,j)%pflx_input_litr_cwd = biophysforc%p31flx%pflx_input_litr_cwd_vr_col(c,j)
      this%summsforc(c,j)%pflx_input_litr_fwd = biophysforc%p31flx%pflx_input_litr_fwd_vr_col(c,j)
      this%summsforc(c,j)%pflx_input_litr_lwd = biophysforc%p31flx%pflx_input_litr_lwd_vr_col(c,j)

      !mineral nutrient input
      this%summsforc(c,j)%sflx_minn_input_nh4 = biophysforc%n14flx%nflx_minn_input_nh4_vr_col(c,j)     !nh4 from deposition and fertilization
      this%summsforc(c,j)%sflx_minn_input_no3 = biophysforc%n14flx%nflx_minn_input_no3_vr_col(c,j)
      this%summsforc(c,j)%sflx_minn_nh4_fix_nomic = biophysforc%n14flx%nflx_minn_nh4_fix_nomic_vr_col(c,j)       !nh4 from fixation
      this%summsforc(c,j)%sflx_minp_input_po4 = biophysforc%p31flx%pflx_minp_input_po4_vr_col(c,j)     !inorganic P from deposition and fertilization
      this%summsforc(c,j)%sflx_minp_weathering_po4 = biophysforc%p31flx%pflx_minp_weathering_po4_vr_col(c,j) 
      this%summsforc(c,j)%biochem_pmin = biophysforc%biochem_pmin_vr(c,j)
    ...
    ! end of the testing
      ...
      this%summsforc(c,j)%bd   = biophysforc%bd_col(c,j)              !bulk density
      this%summsforc(c,j)%pct_sand = biophysforc%cellsand_col(c,j)
      this%summsforc(c,j)%pct_clay = biophysforc%cellclay_col(c,j)
      this%summsforc(c,j)%h2osoi_vol = biophysforc%h2osoi_vol_col(c,j)
      this%summsforc(c,j)%h2osoi_liq = biophysforc%h2osoi_liq_col(c,j)
      this%summsforc(c,j)%air_vol = biophysforc%air_vol_col(c,j)
      this%summsforc(c,j)%finundated = biophysforc%finundated_col(c)
      this%summsforc(c,j)%watsat = biophysforc%watsat_col(c,j)
      this%summsforc(c,j)%watfc = biophysforc%watfc_col(c,j)
     ...
     ! if (this%record == 0 .and. j < 3)then
      ...
      this%summsforc(c,j)%aren_cond_o2 = &
      ...
      this%summsforc(c,j)%aren_cond_n2o = &
      ...
      this%summsforc(c,j)%aren_cond_co2 = &
      ...
      if(this%use_c13)then
        this%summsforc(c,j)%aren_cond_co2_c13 = &
      ...
      if(this%use_c14)then
        this%summsforc(c,j)%aren_cond_co2_c14 = &
      ...
      this%summsforc(c,j)%aren_cond_ar = &
 ...
 !end select
  ...
  associate(                              &
    ...
    nsummspools => this%summsbgc_index%nom_pools, &
    is_sumpool_som => this%summsbgc_index%is_sumpool_som &
  )
  do jj = 1, nsummspools
    kc = (jj-1) * nelms + c_loc
    kn = (jj-1) * nelms + n_loc
    kp = (jj-1) * nelms + p_loc
    if( ystatesf(kc) <= tiny_val  .and. is_sumpool_som(jj))then
      ystatesf(kc)=0._r8
      ystatesf(kn)=0._r8
      ystatesf(kp)=0._r8
      if(this%use_c14)then
...
!write(stdout, *) 'this%summsforc(c,j)%ystates(Bm_end)=', this%summsforc(c,j)%ystates(Bm_end)
      ...
      tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_ch4) = &
        ystatesf(this%summsbgc_index%lid_ch4)

      if(this%non_limit)then

         if(ystatesf(this%summsbgc_index%lid_nh4)>0._r8)then
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_nh3x) = &
            ystatesf(this%summsbgc_index%lid_nh4)
          ...
          biogeo_flux%n14flux_vars%supplement_to_sminn_vr_col(c,j) = &
              -ystatesf(this%summsbgc_index%lid_nh4)*natomw/dtime
          ystatesf(this%summsbgc_index%lid_nh4)=0._r8
        endif

         if(ystatesf(this%summsbgc_index%lid_no3)>0._r8)then
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_no3x) = &
            ystatesf(this%summsbgc_index%lid_no3)
          ...
          biogeo_flux%n14flux_vars%supplement_to_sminn_vr_col(c,j) =    &
             biogeo_flux%n14flux_vars%supplement_to_sminn_vr_col(c,j) - &
             ystatesf(this%summsbgc_index%lid_no3)*natomw/dtime
          ystatesf(this%summsbgc_index%lid_no3) = 0._r8
        endif
      else
        tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_nh3x) = &
          ystatesf(this%summsbgc_index%lid_nh4)
      ...
      tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_n2o) = &
        ystatesf(this%summsbgc_index%lid_n2o)
      ...
      if(.not. fix_ip)then
        ...
        if(this%nop_limit)then
          ...
          if(ystatesf(this%summsbgc_index%lid_minp_soluble)>0._r8)then
            tracerstate_vars%tracer_conc_mobile_col(c,j,betrtracer_vars%id_trc_p_sol) = &
              ystatesf(this%summsbgc_index%lid_minp_soluble)
            ...
            biogeo_flux%p31flux_vars%supplement_to_sminp_vr_col(c,j) = 0._r8 &
              -ystatesf(this%summsbgc_index%lid_minp_soluble)*patomw/dtime
            ystatesf(this%summsbgc_index%lid_minp_soluble) = 0._r8
          endif
        else
          tracerstate_vars%tracer_conc_mobile_col(c,j,betrtracer_vars%id_trc_p_sol) = &
            ystatesf(this%summsbgc_index%lid_minp_soluble)
        ...
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_p_sol) =      &
          ystatesf(this%summsbgc_index%lid_minp_soluble) - &
          ystates0(this%summsbgc_index%lid_minp_soluble)

        trcid = betrtracer_vars%id_trc_beg_minp
        tracer_flx_netpro_vr(c,j, trcid) = &
          ystatesf(this%summsbgc_index%lid_minp_secondary) - &
          ystates0(this%summsbgc_index%lid_minp_secondary)

        trcid = betrtracer_vars%id_trc_end_minp
        tracer_flx_netpro_vr(c,j, trcid) =  &
          ystatesf(this%summsbgc_index%lid_minp_occlude) - &
          ystates0(this%summsbgc_index%lid_minp_occlude)
      endif
      !tracer fluxes
      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_nh3x) =  &
        ystatesf(this%summsbgc_index%lid_nh4) - &
        ystates0(this%summsbgc_index%lid_nh4)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_no3x)  =  &
        ystatesf(this%summsbgc_index%lid_no3) - &
        ystates0(this%summsbgc_index%lid_no3)

      tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_o2) ) = &
         ystatesf(this%summsbgc_index%lid_o2_paere )  - &
         ystates0(this%summsbgc_index%lid_o2_paere)

      if ( betr_spinup_state == 0 ) then
        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_n2)  ) = &
          ystatesf(this%summsbgc_index%lid_n2_paere)  - &
          ystates0(this%summsbgc_index%lid_n2_paere)

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_ar)  ) = &
          ystatesf(this%summsbgc_index%lid_ar_paere)  - &
          ystates0(this%summsbgc_index%lid_ar_paere)

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_co2x)) = &
          ystatesf(this%summsbgc_index%lid_co2_paere)  - &
          ystates0(this%summsbgc_index%lid_co2_paere)

        if(this%use_c13)then
          tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_c13_co2x)) = &
            ystatesf(this%summsbgc_index%lid_c13_co2_paere)  - &
            ystates0(this%summsbgc_index%lid_c13_co2_paere)
        endif

        if(this%use_c14)then
          tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_c14_co2x)) = &
            ystatesf(this%summsbgc_index%lid_c14_co2_paere)  - &
            ystates0(this%summsbgc_index%lid_c14_co2_paere)
        endif

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_ch4) ) = &
          ystatesf(this%summsbgc_index%lid_ch4_paere)  - &
          ystates0(this%summsbgc_index%lid_ch4_paere)

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_n2o) ) = &
          ystatesf(this%summsbgc_index%lid_n2o_paere)  - &
          ystates0(this%summsbgc_index%lid_n2o_paere)
      endif

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_n2) = &
        ystatesf(this%summsbgc_index%lid_n2) - &
        ystates0(this%summsbgc_index%lid_n2)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_co2x ) = &
        ystatesf(this%summsbgc_index%lid_co2) - &
        ystates0(this%summsbgc_index%lid_co2)

      if(this%use_c13)then
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_c13_co2x ) = &
          ystatesf(this%summsbgc_index%lid_c13_co2) - &
          ystates0(this%summsbgc_index%lid_c13_co2)
      endif

      if(this%use_c14)then
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_c14_co2x ) = &
          ystatesf(this%summsbgc_index%lid_c14_co2) - &
          ystates0(this%summsbgc_index%lid_c14_co2)
      endif

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_n2o  ) = &
        ystatesf(this%summsbgc_index%lid_n2o) - &
        ystates0(this%summsbgc_index%lid_n2o)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_o2   ) = &
        ystatesf(this%summsbgc_index%lid_o2) - &
        ystates0(this%summsbgc_index%lid_o2)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_ch4  ) = &
        ystatesf(this%summsbgc_index%lid_ch4) - &
        ystates0(this%summsbgc_index%lid_ch4)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_ar) = &
        ystatesf(this%summsbgc_index%lid_ar) - &
        ystates0(this%summsbgc_index%lid_ar)
     ...
     ! if(j<=3)then 
      ...
      biogeo_flux%n14flux_vars%f_denit_vr_col(c,j)= &
        (ystatesf(this%summsbgc_index%lid_no3_den) - &
         ystates0(this%summsbgc_index%lid_no3_den))*natomw/dtime

      biogeo_flux%n14flux_vars%f_nit_vr_col(c,j) = &
        (ystatesf(this%summsbgc_index%lid_nh4_nit) - &
         ystates0(this%summsbgc_index%lid_nh4_nit))*natomw/dtime

      biogeo_flux%n14flux_vars%f_n2o_nit_vr_col(c,j) = &
        (ystatesf(this%summsbgc_index%lid_n2o_nit) - &
         ystates0(this%summsbgc_index%lid_n2o_nit))*natomw/dtime

  select type(plant_soilbgc)
  type is(plant_soilbgc_summs_type)
    do p = 1, this%nactpft
      plant_soilbgc%plant_minn_no3_active_yield_flx_vr_patch(p,j) = &
          (ystatesf(this%summsbgc_index%lid_plant_minn_no3_pft(p)) - &
          ystates0(this%summsbgc_index%lid_plant_minn_no3_pft(p)))*natomw/dtime

      plant_soilbgc%plant_minn_nh4_active_yield_flx_vr_patch(p,j) = &
          (ystatesf(this%summsbgc_index%lid_plant_minn_nh4_pft(p)) - &
          ystates0(this%summsbgc_index%lid_plant_minn_nh4_pft(p)))*natomw/dtime

      plant_soilbgc%plant_minp_active_yield_flx_vr_patch(p,j) = &
          (ystatesf(this%summsbgc_index%lid_plant_minp_pft(p)) - &
           ystates0(this%summsbgc_index%lid_plant_minp_pft(p)))*patomw/dtime

    enddo

    plant_soilbgc%plant_minn_no3_active_yield_flx_vr_col(c,j) = &
          (ystatesf(this%summsbgc_index%lid_plant_minn_no3) - &
          ystates0(this%summsbgc_index%lid_plant_minn_no3))*natomw/dtime

    plant_soilbgc%plant_minn_nh4_active_yield_flx_vr_col(c,j) = &
          (ystatesf(this%summsbgc_index%lid_plant_minn_nh4) - &
          ystates0(this%summsbgc_index%lid_plant_minn_nh4))*natomw/dtime

    plant_soilbgc%plant_minp_active_yield_flx_vr_col(c,j) = &
          (ystatesf(this%summsbgc_index%lid_plant_minp) - &
           ystates0(this%summsbgc_index%lid_plant_minp))*patomw/dtime
